...
export function StrategicTab() {
  ...
  const regimeLabels: Record<string, TranslationKey> = {
    bull: "strat.bull",
    bear: "strat.bear",
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${BASE_URL}/api/fund/strategic/overview`)
        .then((r) => r.json())
        .then(setOverview),
      fetch(`${BASE_URL}/api/fund/strategic/thesis-health`)
        .then((r) => r.json())
        .then((d) => setTheses(d.theses || [])),
      fetch(`${BASE_URL}/api/fund/strategic/social-signals`)
        .then((r) => r.json())
  ...
  const regimeCfg =
    REGIME_CONFIG[regime as keyof typeof REGIME_CONFIG] ??
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={`text-2xl font-bold ${regimeCfg.color}`}>
                {t(regimeLabels[regime] ?? "strat.normal")}
              </p>
              <p className="text-sm text-[#8b949e]">
                {t("strat.confidence")}:{" "}
                {((overview?.regime_confidence ?? 0) * 100).toFixed(0)}% |{" "}
                {t("strat.avg_daily")}:{" "}
                {overview?.avg_daily_return_pct &&
                overview.avg_daily_return_pct >= 0
                  ? "+"
                  : ""}
                {(overview?.avg_daily_return_pct ?? 0).toFixed(3)}%
              </p>
            </div>
          </div>
        ...
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2 text-xs">
              <span className="text-[#00d4aa]">
                {theses.filter((th) => th.status === "active").length}{" "}
                {t("strat.active")}
              </span>
              <span className="text-yellow-400">
                {theses.filter((th) => th.status === "flagged").length}{" "}
                {t("strat.flagged")}
              </span>
              <span className="text-[#ff6b6b]">
                {theses.filter((th) => th.status === "invalidated").length}{" "}
                {t("strat.invalid")}
              </span>
            </div>
          </div>
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {theses.map((th) => (
              <div
                key={th.pm_id}
                className="flex items-center gap-3 p-2 bg-[#1c2128] rounded-lg"
              >
                <span>{th.pm_emoji}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white truncate">
                    {th.pm_name}
                  </p>
                  <p className="text-xs text-[#8b949e] truncate">{th.thesis}</p>
                </div>
                <span
                  className={`text-xs px-2 py-0.5 rounded border flex-shrink-0 ${STATUS_STYLES[th.status]}`}
                >
                  {th.status}
                </span>
              </div>
        ...
        <div className="glass-card p-5">
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {socialSignals.slice(0, 8).map((sig) => (
              <div
                key={sig.symbol}
                className={`p-3 rounded-lg border ${sig.is_tipping_point ? "bg-[#ff6b6b]/10 border-[#ff6b6b]/30" : "bg-[#1c2128] border-[#30363d]"}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded ${sig.direction === "bullish" ? "bg-[#00d4aa]/20 text-[#00d4aa]" : sig.direction === "bearish" ? "bg-[#ff6b6b]/20 text-[#ff6b6b]" : "bg-gray-500/20 text-gray-400"}`}
                    >
                      {sig.direction}
                    </span>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-mono text-[#8b949e]">
                      Z: {sig.zscore.toFixed(1)}σ
                    </p>
                    <p className="text-xs text-[#8b949e]">
                      Reddit: {sig.reddit_mentions}
                    </p>
                  </div>
                </div>
                {sig.is_tipping_point && (
                  <p className="text-xs text-[#ff6b6b] mt-1">
                    ⚡ {t("strat.tipping")} Conviction:{" "}
      ...
      <div className="glass-card p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {overview?.pms?.map((pm) => (
            <div
              key={pm.id}
              className="flex items-start gap-3 p-3 bg-[#1c2128] rounded-lg"
            >
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span
                    className={`text-xs font-mono ${pm.itd_return >= 0 ? "text-[#00d4aa]" : "text-[#ff6b6b]"}`}
                  >
                    {pm.itd_return >= 0 ? "+" : ""}
                    {pm.itd_return.toFixed(1)}%
                  </span>
                </div>
